...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await authAPI.registerViewer(formData);
      await login(formData.email, formData.password);
      toast.success('Account created! You are now logged in as a viewer.');
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-900 p-4">
      <div className="w-full max-w-md">
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl p-8">
          ...
          <h1 className="text-2xl font-bold text-center text-slate-900 dark:text-white mb-2">Create Viewer Account</h1>
          <p className="text-sm text-center text-slate-600 dark:text-slate-400 mb-6">
            Sign up to access the admin panel as a read‑only viewer
          </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              ...
              disabled={loading}
            >
              {loading ? 'Creating...' : <><FaUserPlus /> Create Viewer Account</>}
            </button>
          </form>
          ...
          <p className="mt-2 text-center text-xs text-slate-500 dark:text-slate-500">
            Viewer accounts have read‑only access to the admin panel.
          </p>
        </div>
      </div>
    </div>
